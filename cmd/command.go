...
func main() {
	...
	if len(args) > 1 {
		...
		if args[1] == "user" {
			uname := string(common.GenerateRandomString(6, common.T_RAND_LOWER))
			pass := "123456"
			if len(args) > 3 {
				uname = args[2]
				pass = args[3]
...
func Help() {
	fmt.Println("migrate					sync the model to database\r\n" +
...
type MigrateList map[string]interface{}

func LoadMigrateList() {
...
func ClientMake(url string) {
	...
	id := string(common.GenerateRandomString(12, common.T_RAND_LOWERNUM))
	client.ClientUser = model.C_PREFIX_WEB + id
	client.Secret = string(common.GenerateRandomString(128))
...
func RandomUser(num string) {
	...
	if e == nil {
		for ; n > 0 && n < MAX; n-- {
			...
			user.Username = string(common.GenerateRandomString(6, common.T_RAND_LOWER))
			time.Sleep(1)
			user.Nickname = string(common.GenerateRandomString(12, common.T_RAND_UPPER))
			user.Mail = user.Nickname + "@mana.com"
			user.Mobile = "130" + string(common.GenerateRandomString(8, common.T_RAND_NUM))
			Random(&user)
		}
	}
